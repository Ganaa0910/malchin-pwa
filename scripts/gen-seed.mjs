...
const SHEEP_NAMES = [
  "Мангалай", "Цоохор", "Хонгил", "Сахалт", "Шурх", "Цагаан хошуу",
...
breachAnimals.forEach((a, i) => {
  ALERTS.push({
    ...
    type: "breach",
    priority: "critical",
    title: `${a.name ?? a.id} хязгаар давсан`,
    message: `${a.species} ${a.id} үндсэн бэлчээрийн хязгаараас ${Math.round(
      a.distanceFromBaseM,
    )} м-ийн зайд. Яаралтай эргүүлэх шаардлагатай.`,
    animalId: a.id,
    deviceId: a.deviceId,
    zoneId: "zone-main",
...
lowBatt.forEach((d, i) => {
  ...
  ALERTS.push({
    ...
    type: "low-battery",
    priority: "high",
    title: `${d.id} батарей бага: ${d.battery}%`,
    message: `${a?.species ?? "Мал"} ${a?.id ?? ""} -н GPS төхөөрөмжийн цэнэг ${d.battery}% болсон. Цэнэглэх шаардлагатай.`,
    animalId: a?.id ?? null,
    deviceId: d.id,
    zoneId: null,
...
ALERTS.push({
  ...
  type: "predator",
  priority: "critical",
  title: "Чоно ойртсон шинж",
  message:
    "Сүргийн хойд талд (1.2 км) хурдны муруйлт ажиглагдсан. Чоно ойртсон байж магадгүй.",
  animalId: null,
  deviceId: null,
  zoneId: "zone-main",
...
ALERTS.push({
  ...
  type: "base-station",
  priority: "medium",
  title: "Баазын станц богино хугацаанд тасарсан",
  message: "BS-001 баазын станц 6 минут офлайн байсан. Одоо хэвийн ажиллаж байна.",
  animalId: null,
  deviceId: "BS-001",
  zoneId: null,
...
ALERTS.push({
  ...
  type: "weather",
  priority: "medium",
  title: "Цас орох магадлал өндөр",
  message:
    "Маргааш орой 15-20 см цас орох төлөвтэй. Зуданы эрсдэлд бэлдэх шаардлагатай.",
  animalId: null,
  deviceId: null,
  zoneId: null,
...
if (lameAnimal) {
  ALERTS.push({
    ...
    message:
      "GPS-ийн дагуу хөдөлгөөн удаашрсан. Биеийн байдлыг шалгах хэрэгтэй.",
